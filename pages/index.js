...
export default class Index extends React.Component {
      ...
      if (activeFounder.person === this.state.activeFounder.person) {
        ...
        this.setState({ transitioning: true }, async () => {
          ...
          this.setState({ activeFounder, audioPlaying: true }, () => {
            ...
            setTimeout(() => this.setState({ transitioning: false }), TRANSITION_DURATION / 2)
          })
        })
      }
  }
  ...
  onAudioEnd = () => {
    ...
    const nextFounderIndex = Object.keys(FOUNDERS).indexOf(activeFounder.person) + 1
    const nextFounder = Object.values(FOUNDERS)[nextFounderIndex] || Object.values(FOUNDERS)[0]
        ...
        <div className="audios">
          {
            Object.keys(FOUNDERS).map(key => (
              <button
                className="audio"
                style={{ backgroundRepeat: `no-repeat`, backgroundSize: `100% 100%`, backgroundImage: `url('${FOUNDERS[key].picture}')` }}
                key={key}
